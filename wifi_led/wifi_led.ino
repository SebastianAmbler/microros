...
#include <std_msgs/msg/int32.h>   // <-- LED command message

// ===================== WiFi + micro-ROS agent config =====================

const char* WIFI_SSID     = "EMB5325";
const char* WIFI_PASSWORD = "cdti12345";
const char* AGENT_IP      = "192.168.1.145";   // ROS 2 PC/RPi IP
...
// Motor control pins (H-bridge / driver) - DIRECTION
const int INA1 = 33; 
const int INB1 = 25;
const int INA2 = 12;  
const int INB2 = 14;

// Motor PWM pins (connect to ENA / ENB of motor driver)
const int PWM_LEFT_PIN  = 32; // TODO: set to actual left enable pin
const int PWM_RIGHT_PIN = 13; // TODO: set to actual right enable pin

// LED pins (R/Y/G)
const int LED_R_PIN = 17;  // Red
const int LED_Y_PIN = 16;  // Yellow
const int LED_G_PIN = 4;   // Green
...
// ===== LED control helper =====
// mask bits: bit0=R, bit1=Y, bit2=G
void set_leds_from_mask(uint8_t mask) {
  digitalWrite(LED_R_PIN, (mask & 0x01) ? HIGH : LOW);
  digitalWrite(LED_Y_PIN, (mask & 0x02) ? HIGH : LOW);
  digitalWrite(LED_G_PIN, (mask & 0x04) ? HIGH : LOW);
}

// Motor control: left + right speeds in [-100, 100]
void move(int leftspeed, int rightspeed) {
  // Invert LEFT motor sign to fix its direction
  leftspeed = -leftspeed;
  ...
  // Apply PWM to dedicated enable pins
...
void led_cmd_callback(const void *msgin) {
  ...
  uint8_t mask = (uint8_t)(msg->data & 0x07);  // only use lower 3 bits
...
void setup() {
  ...
  set_leds_from_mask(0);  // all off at start
  ...
  // executor with 1 timer + 2 subscribers (cmd_vel + led_cmd)
  ...
  Serial.println("Robot initialized: WiFi micro-ROS, /cmd_vel + /led_cmd subscribers, /odom + /tf publisher");
}
